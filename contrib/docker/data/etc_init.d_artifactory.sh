...
to your /etc/security/limits.conf file."
        ...
        findShutdownPort
        nbSeconds=1
        while [ $SHUTDOWN_PORT -eq 0 ] && [ $nbSeconds -lt $STARTUP_WAIT_SECS ]; do
            sleep 1
        ...
        if [ $SHUTDOWN_PORT -eq 0 ]; then
            errorArtHome "Artifactory Tomcat server did not start in $STARTUP_WAIT_SECS seconds. Please check the logs"
        fi
...
ARTIFACTORY_PID=""
artDefaultPath="/etc/opt/jfrog/artifactory"
artDefaultFile="${artDefaultPath}/default"
...
# just in case $ARTIFACTORY_HOME/etc/default is not the same as $artDefaultFile
[ -f "$ARTIFACTORY_HOME/etc/default" ] && source "$ARTIFACTORY_HOME/etc/default"

# populate $ARTIFACTORY_HOME/etc with defaults if not present.
cp -vn "$artDefaultPath"/* "$ARTIFACTORY_HOME/etc/"
chmod -v a+rw "$ARTIFACTORY_HOME/etc"/*
